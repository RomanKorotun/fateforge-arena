...
export class PlaySpinUseCase {
  ...
  async execute({ userId, data }: PlaySpinCommand) {
    ...
    // 2. Вся фінансова логіка виконується в одній транзакції.
    //
    // lockById() використовує SELECT ... FOR UPDATE,
    // тому рядок гаманця блокується до завершення транзакції.
    //
    // Якщо два spin-запити для одного гаманця прийдуть одночасно:
    //
    // 2.1. Перша транзакція захопить блокування.
    // 2.2. Друга буде чекати звільнення блокування.
    // 2.3. Після завершення першої транзакції друга
    //    прочитає вже оновлений баланс.
    //
    // Це запобігає race condition та double spending,
    // коли один і той самий баланс може бути витрачений двічі.
